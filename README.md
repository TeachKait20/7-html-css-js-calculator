...
**3.** `sNaN(value)` - Проверяет, является ли значение NaN (не числом).
...
**6.** `addEventListener('click', calculate)` - Добавляет событие клика на кнопку с вызовом функции calculate.
