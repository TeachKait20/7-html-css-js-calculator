...
* `let result;` - Создаёт переменную для хранения результата вычисления. <br>
...
**4.** `switch (selectedOperator)` - Проверяет значение оператора и выполняет соответствующую операцию (аналог `if`): <br>
...
**5.** `output.textContent` - Устанавливает текст внутри элемента с id="out", который отобразит результат. <br>

**6.** `addEventListener('click', calculate)` - Добавляет событие клика на кнопку с вызовом функции calculate. <br>
Когда пользователь нажимает кнопку "=", выполняется функция calculate, и результат отображается на экране. <br>
